...
public class EthUtils {
    ...
    public static String sendErc20(Web3j web3j, String contractAddress, String privateKey,
        ...
        try {
            ...
            System.out.println(String.format("发送ERC20失败 from=%s to=%s erc20=%s amount=%s", from, to, contractAddress, value.toString()) + t);
        }
        return null;
    }
...
*/
  ...
  public static void main(String[] args) throws IOException {
    Web3j ETHweb3j = Web3j.build(new HttpService("http://127.02.02.02:8445"));
    long nowBlockNumber = getNowBlockNumber(ETHweb3j);
    System.out.println("nowBlockNumber = " + nowBlockNumber);
    EthBlock block = getBlock(ETHweb3j, nowBlockNumber, true);
    String hash = block.getResult().getHash();
    System.out.println("block = " + hash);
    BigInteger number = block.getResult().getDifficulty();
    System.out.println("number = " + number);
    //String author = block.getResult().getLogsBloom();
    //System.out.println("block = " + author);
    BigInteger size = block.getResult().getSize();
    System.out.println("size = " + size);
    //获取block
    EthBlock.Block block1 = ETHweb3j.ethGetBlockByNumber(DefaultBlockParameter.valueOf(BigInteger.valueOf(nowBlockNumber)), true).send().getBlock();
    List<EthBlock.TransactionResult> transactions = block.getResult().getTransactions();
    // 遍历block中的交易
    for (EthBlock.TransactionResult tx : block1.getTransactions()) {
